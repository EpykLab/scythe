...
class JourneyExecutor:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Expected Result: {'SUCCESS' if self.journey.expected_result else 'FAILURE'}"
        )
        ...
        try:
            if self.mode == "API":
                ...
                if getattr(self.journey, "authentication", None):
                    ...
                    try:
                        auth_headers = (
                            self.journey.authentication.get_auth_headers() or {}
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to get auth headers from authentication: {e}"
                        )
                    try:
                        if hasattr(self.journey.authentication, "get_auth_cookies"):
                            auth_cookies = (
                                self.journey.authentication.get_auth_cookies() or {}
                            )
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to get auth cookies from authentication: {e}"
                        )
                if auth_headers:
                ...
                if (
                    hasattr(self.journey.authentication, "_session")
                    and self.journey.authentication._session
                ):
                    try:
                ...
                if getattr(self.journey, "csrf_protection", None):
                    self.journey.set_context(
                        "csrf_protection", self.journey.csrf_protection
                    )
                    self.logger.info(
                        f"CSRF protection enabled: {self.journey.csrf_protection}"
                    )
                ...
                # Execute the journey
                if self.driver:
                    self.execution_results = self.journey.execute(
            ...
            self.logger.error(
                f"An unexpected error occurred during journey execution: {e}",
                exc_info=True,
            )
            if not self.execution_results:
    ...
    def _apply_behavior_to_journey(self):
        ...
        for i, step_result in enumerate(
            self.execution_results.get("step_results", []), 1
        ):
            ...
            if hasattr(self.behavior, "get_step_delay"):
                ...
                if delay > 0:
                    self.logger.info(
                        f"Behavior delay: {delay:.2f}s after step '{step_name}'"
                    )
            ...
            if hasattr(self.behavior, "post_step"):
                try:
                    ...
                    if self.driver:
                        self.behavior.post_step(
                            self.driver, mock_payload, i, step_success
                        )
                except Exception as e:
    ...
    def _cleanup(self):
        ...
        if overall_success == expected:
            if expected:
                self.logger.info(
                    f"✓ EXPECTED SUCCESS: Journey completed successfully in {execution_time:.2f}s"
                )
            else:
                self.logger.info(
                    f"✓ EXPECTED FAILURE: Journey failed as expected in {execution_time:.2f}s"
                )
        else:
            if expected:
                self.logger.warning(
                    f"✗ UNEXPECTED FAILURE: Journey was expected to succeed but failed in {execution_time:.2f}s"
                )
            else:
                self.logger.warning(
                    f"✗ UNEXPECTED SUCCESS: Journey was expected to fail but succeeded in {execution_time:.2f}s"
                )
        ...
        self.logger.info(
            f"Steps: {steps_succeeded}/{steps_executed} succeeded ({steps_failed} failed)"
        )
        ...
        self.logger.info(
            f"Actions: {actions_succeeded}/{actions_executed} succeeded ({actions_failed} failed)"
        )
        ...
        if step_results:
            ...
            for i, step_result in enumerate(step_results, 1):
                ...
                expected_text = (
                    "expected" if step_success == step_expected else "unexpected"
                )
                version_info = f" | Version: {target_version}" if target_version else ""

                self.logger.info(
                    f"  {status} Step {i}: {step_name} - {result_text} ({expected_text}){version_info}"
                )
                self.logger.info(
                    f"    Actions: {len([a for a in actions if a.get('actual', False)])}/{len(actions)} succeeded"
                )
                ...
                for a in actions:
                    ...
                    if actual != expected:
                        prefix = (
                            "✗ Action failed"
                            if expected
                            else "✗ Action unexpectedly succeeded"
                        )
                        ...
                        if ad.get("request_headers"):
                            self.logger.error(
                                f"      Request headers: {ad.get('request_headers')}"
                            )
                        if ad.get("request_params"):
                            self.logger.error(
                                f"      Request params: {ad.get('request_params')}"
                            )
                        if ad.get("request_json") is not None:
                            self.logger.error(
                                f"      Request JSON: {ad.get('request_json')}"
                            )
                        if ad.get("request_data") is not None:
                            self.logger.error(
                                f"      Request data: {ad.get('request_data')}"
                            )
                        if ad.get("response_headers"):
                            self.logger.error(
                                f"      Response headers: {ad.get('response_headers')}"
                            )
                        if "response_json" in ad:
                            self.logger.error(
                                f"      Response JSON: {ad.get('response_json')}"
                            )
                        elif "response_text" in ad:
                            self.logger.error(
                                f"      Response text: {ad.get('response_text')}"
                            )
                        if ad.get("error"):
        ...
        if target_versions:
            ...
            self.logger.info(
                f"  Steps with version info: {len(target_versions)}/{len(step_results) if step_results else 0}"
            )
...
class JourneyRunner:
    ...
    def __init__(
        self, headless: bool = True, behavior: Optional[Behavior] = None, sleep_fn=None
    ):
        """
    ...
    def run_journey(
        self, journey: Journey, target_url: str, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def run_journeys(
        self, journeys: List[tuple], delay_between: float = 1.0
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _print_batch_summary(self, results: List[Dict[str, Any]]):
        ...
        successful_journeys = sum(
            1 for r in results if r.get("overall_success") == r.get("expected_result")
        )
        total_time = sum(r.get("execution_time", 0) for r in results)

        self.logger.info(
            f"Journeys: {successful_journeys}/{total_journeys} completed as expected"
        )
        self.logger.info(f"Total execution time: {total_time:.2f}s")
        self.logger.info(
            f"Average time per journey: {total_time / total_journeys:.2f}s"
        )
        ...
        for i, result in enumerate(results, 1):
            ...
            self.logger.info(
                f"  {status} {journey_name}: {result_text} ({expected_text}) - {exec_time:.2f}s"
            )
