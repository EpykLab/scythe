...
class Step:
    ...
    def execute(self, driver: WebDriver | None, context: Dict[str, Any]) -> bool:
        ...
        # Reset step-scoped execution state for each run
        self.execution_results = []
        self.step_data = {}
            ...
            try:
                ...
                if result:
                    if action.expected_result:
                        ...
                        logger.warning(
                            f"✗ Action unexpectedly succeeded: {action.name}"
                        )
                        success_count += 1
                else:
                    if action.expected_result:
                        ...
                        try:
                            ...
                            method = ad.get("request_method") or getattr(
                                action, "method", None
                            )
                ...
                try:
                    ...
                    if details:
                        method = details.get("request_method") or getattr(
                            action, "method", None
                        )
                        ...
                        if details.get("request_headers"):
                            logger.error(
                                f"    Request headers: {details.get('request_headers')}"
                            )
                        if details.get("response_headers"):
                            logger.error(
                                f"    Response headers: {details.get('response_headers')}"
                            )
                        if "response_json" in details:
                            logger.error(
                                f"    Response JSON: {details.get('response_json')}"
                            )
                        elif "response_text" in details:
                            logger.error(
                                f"    Response text: {details.get('response_text')}"
                            )
                except Exception:
                    pass
                failure_count += 1
        ...
        step_success = failure_count == 0 or (
            self.continue_on_failure and success_count > 0
        )

        logger.info(
            f"Step completed: {success_count} successes, {failure_count} failures"
        )
        return step_success
...
class Journey:
    ...
    def execute(self, driver: WebDriver | None, target_url: str) -> Dict[str, Any]:
        ...
        self.execution_results = []
        time_fn = execution_context.get("_time_fn", time.time)
        sleep_fn = execution_context.get("_sleep_fn", time.sleep)
        ...
        normalized_target_url = (
            HeaderExtractor._normalize_url(target_url)
            if isinstance(target_url, str)
            else target_url
        )
        execution_context["target_url"] = normalized_target_url
        execution_context["journey_name"] = self.name
        ...
        try:
            ...
            if self.requires_authentication():
                auth_name = (
                    self.authentication.name if self.authentication else "Unknown"
                )
                ...
                if driver is None:
                    ...
                    headers = {}
                    try:
                        if self.authentication and hasattr(
                            self.authentication, "get_auth_headers"
                        ):
                    ...
                    try:
                        ...
                        cookies = {}
                    if cookies:
                        existing_cookies = execution_context.get("auth_cookies", {})
                        merged_cookies = {**existing_cookies, **cookies}
                        execution_context["auth_cookies"] = merged_cookies
                        logger.info("Authentication cookies prepared for API mode")
                    if not headers and not cookies:
                        logger.error(
                            "Authentication required but no headers/cookies available in API mode"
                        )
                        results["errors"].append(
                            "Authentication failed (no API auth data)"
                        )
                        return results
                else:
            ...
            for i, step in enumerate(self.steps, 1):
                ...
                try:
                    step_success = step.execute(driver, execution_context)
                    ...
                    target_version = header_extractor.extract_target_version_hybrid(
                        driver, target_url
                    )
                    if target_version:
                    ...
                    try:
                        ...
                        resume_at = execution_context.get("rate_limit_resume_at")
                        now = time_fn()
                        if isinstance(resume_at, (int, float)) and resume_at > now:
                            wait_s = min(resume_at - now, 30)
                            if wait_s > 0:
                                logger.info(
                                    f"Rate limit backoff in effect; waiting {wait_s:.2f}s before next step"
                                )
                                sleep_fn(wait_s)
                        else:
                            last_headers = (
                                execution_context.get("last_response_headers") or {}
                            )
                            if isinstance(last_headers, dict) and last_headers:
                                ...
                                if retry_after is not None:
                                    try:
                                        ...
                                        if wait_s > 0:
                                            logger.info(
                                                f"Rate-limited by server (Retry-After={wait_s}s); pausing before next step"
                                            )
                                            sleep_fn(min(wait_s, 30))
                                    except Exception:
                                        pass
                                else:
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            if wait_s > 0:
                                                logger.info(
                                                    f"Rate limit reached (remaining=0). Waiting {wait_s}s for reset before next step"
                                                )
                                                sleep_fn(min(wait_s, 30))
                                        except Exception:
                                            pass
            ...
            if results["target_versions"]:
                version_summary = header_extractor.get_version_summary(
                    [{"target_version": v} for v in results["target_versions"]]
                )
                results["version_summary"] = version_summary
                logger.info(
                    f"Target versions detected: {list(set(results['target_versions']))}"
                )
            ...
            logger.info(
                f"Steps: {results['steps_succeeded']}/{results['steps_executed']} succeeded"
            )
            logger.info(
                f"Actions: {results['actions_succeeded']}/{results['actions_executed']} succeeded"
            )
