...
class ScaleOrchestrator(Orchestrator):
    ...
    def orchestrate_ttp(
        ...
        try:
            if self.strategy == OrchestrationStrategy.SEQUENTIAL:
                results, errors = self._execute_ttp_sequential(
                    ttp, target_url, replications, **kwargs
                )
            elif self.strategy == OrchestrationStrategy.PARALLEL:
                results, errors = self._execute_ttp_parallel(
                    ttp, target_url, replications, **kwargs
                )
            else:
    ...
    def orchestrate_journey(
        ...
        try:
            if self.strategy == OrchestrationStrategy.SEQUENTIAL:
                results, errors = self._execute_journey_sequential(
                    journey, target_url, replications, **kwargs
                )
            elif self.strategy == OrchestrationStrategy.PARALLEL:
                results, errors = self._execute_journey_parallel(
                    journey, target_url, replications, **kwargs
                )
            else:
                raise OrchestrationError(
        ...
        for i in range(replications):
            ...
            try:
                ...
                if i > 0 and self.ramp_up_delay > 0:
                    self.logger.info(
                        f"Ramp-up delay: {self.ramp_up_delay}s before execution {i + 1}"
                    )
                ...
                if self.cool_down_delay > 0:
                    self.logger.info(
                        f"Cool-down delay: {self.cool_down_delay}s after execution {i + 1}"
                    )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i in range(replications):
                ...
                future = executor.submit(
                    self._execute_single_ttp, ttp, target_url, context, **kwargs
                )
            ...
            for future in as_completed(future_to_context, timeout=self.timeout):
                ...
                try:
                    ...
                    results.append(result)
                    self.logger.info(
                        f"Completed TTP execution {context.replication_number}/{replications}"
                    )
                    ...
                    error_msg = (
                        f"Error in TTP execution {context.replication_number}: {str(e)}"
                    )
                    self.logger.error(error_msg)
                    errors.append(error_msg)
    ...
    def _execute_journey_sequential(
        ...
        for i in range(replications):
            ...
            try:
                ...
                self.logger.info(
                    f"Executing Journey replication {i + 1}/{replications}"
                )

                # Execute single Journey instance
                result = self._execute_single_journey(
                    journey, target_url, context, **kwargs
                )
                results.append(result)
    ...
    def _execute_journey_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i in range(replications):
                ...
                future = executor.submit(
                    self._execute_single_journey, journey, target_url, context, **kwargs
                )
                future_to_context[future] = context
            ...
            for future in as_completed(future_to_context, timeout=self.timeout):
                ...
                try:
                    ...
                    results.append(result)
                    self.logger.info(
                        f"Completed Journey execution {context.replication_number}/{replications}"
                    )
