...
class DistributedOrchestrator(Orchestrator):
    ...
    def __init__(
        ...
        if not self.proxies:
            self.logger.warning(
                "No proxies configured - tests will run from local network"
            )
        if not self.credentials:
            self.logger.warning(
                "No credentials configured - tests will use default authentication"
            )
    ...
    def add_credential_set(self, credentials: CredentialSet) -> None:
        ...
        self.logger.info(
            f"Added credentials: {credentials.name} ({credentials.username})"
        )

    def orchestrate_ttp(
        self, ttp: TTP, target_url: str, replications: int = 1, **kwargs
    ) -> OrchestrationResult:
        """
        ...
        try:
            if self.strategy == OrchestrationStrategy.SEQUENTIAL:
                results, errors = self._execute_ttp_distributed_sequential(
                    ttp, target_url, replications, **kwargs
                )
            elif self.strategy == OrchestrationStrategy.PARALLEL:
                results, errors = self._execute_ttp_distributed_parallel(
                    ttp, target_url, replications, **kwargs
                )
            else:
                raise OrchestrationError(
                    f"Unsupported strategy: {self.strategy}", self.name
                )
    ...
    def orchestrate_journey(
        self, journey: Journey, target_url: str, replications: int = 1, **kwargs
    ) -> OrchestrationResult:
        """
        ...
        try:
            if self.strategy == OrchestrationStrategy.SEQUENTIAL:
                results, errors = self._execute_journey_distributed_sequential(
                    journey, target_url, replications, **kwargs
                )
            elif self.strategy == OrchestrationStrategy.PARALLEL:
                results, errors = self._execute_journey_distributed_parallel(
                    journey, target_url, replications, **kwargs
                )
            else:
                raise OrchestrationError(
            ...
            self.logger.error(
                f"Critical error in distributed Journey orchestration: {e}"
            )
            errors.append(str(e))
    ...
    def _execute_ttp_distributed_sequential(
        self, ttp: TTP, target_url: str, replications: int, **kwargs
        ...
        for i in range(replications):
            ...
            try:
                ...
                if proxy:
                    self.logger.info(
                        f"  Using proxy: {proxy.name} ({proxy.location or 'unknown location'})"
                    )
                if credentials:
                    self.logger.info(f"  Using credentials: {credentials.name}")

                # Execute single TTP instance
                result = self._execute_single_distributed_ttp(
        ...
        results.sort(key=lambda r: r.get("replication_number", 0))
        return results, errors

    def _execute_ttp_distributed_parallel(
        self, ttp: TTP, target_url: str, replications: int, **kwargs
    ) -> Tuple[List[Dict], List[str]]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i in range(replications):
                ...
                self.logger.info(
                    f"Submitted distributed TTP execution {i + 1}/{replications}"
                )
                if proxy:
            ...
            for future in as_completed(future_to_context, timeout=self.timeout):
                ...
                try:
                    ...
                    results.append(result)
                    self.logger.info(
                        f"Completed distributed TTP execution {context.replication_number}/{replications}"
                    )
    ...
    def _execute_journey_distributed_sequential(
        self, journey: Journey, target_url: str, replications: int, **kwargs
    ) -> Tuple[List[Dict], List[str]]:
        ...
        for i in range(replications):
            ...
            try:
                ...
                self.logger.info(
                    f"Executing distributed Journey {i + 1}/{replications}"
                )
                if proxy:
                    self.logger.info(
                        f"  Using proxy: {proxy.name} ({proxy.location or 'unknown location'})"
                ...
                result = self._execute_single_distributed_journey(
    ...
    def _execute_journey_distributed_parallel(
        self, journey: Journey, target_url: str, replications: int, **kwargs
    ) -> Tuple[List[Dict], List[str]]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i in range(replications):
                ...
                self.logger.info(
                    f"Submitted distributed Journey execution {i + 1}/{replications}"
                )
                if proxy:
                    self.logger.info(f"  Proxy: {proxy.name}")
                if credentials:
            ...
            for future in as_completed(future_to_context, timeout=self.timeout):
                ...
                try:
                    ...
                    results.append(result)
                    self.logger.info(
                        f"Completed distributed Journey execution {context.replication_number}/{replications}"
                    )
    ...
    def _get_execution_resources(
        self, execution_index: int
    ) -> Tuple[Optional[NetworkProxy], Optional[CredentialSet]]:
        ...
        with self.rotation_lock:
            ...
            credentials = (
                self._get_next_credentials(execution_index)
                if self.credentials
                else None
            )
    ...
    def _execute_single_distributed_ttp(
        self,
        ttp: TTP,
        target_url: str,
        context: ExecutionContext,
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ttp_for_run = copy.deepcopy(ttp)
            except Exception:
                ttp_for_run = ttp

            # Configure TTP with distributed credentials if available
            if credentials and hasattr(ttp_for_run, "authentication"):
            ...
            expected_success = ttp_for_run.expected_result

            success = (expected_success and has_results) or (
                not expected_success and not has_results
            )

            result = {
                "execution_id": context.execution_id,
                "replication_number": context.replication_number,
                "test_name": ttp_for_run.name,
                "target_url": target_url,
                "success": success,
    ...
    def _execute_single_distributed_journey(
        self,
        journey: Journey,
        target_url: str,
        context: ExecutionContext,
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                journey_for_run = copy.deepcopy(journey)
            except Exception:
                journey_for_run = journey

            execution_context = journey_for_run.context.copy()

            # Configure Journey with distributed credentials if available
            if credentials:
                # Set credentials in journey context for actions to use
                execution_context["distributed_username"] = credentials.username
                execution_context["distributed_password"] = credentials.password
                if credentials.additional_data:
                    for key, value in credentials.additional_data.items():
                        execution_context[f"distributed_{key}"] = value

            journey_for_run.context = execution_context

            # Create Journey executor with proxy configuration
            executor = JourneyExecutor(
                journey=journey_for_run,
                target_url=target_url,
                headless=self.headless,
                behavior=behavior,
            )
    ...
    def _calculate_distribution_stats(
        ...
        for result in results:
            ...
            if credentials_name:
                credential_usage[credentials_name] = (
                    credential_usage.get(credentials_name, 0) + 1
                )

            proxy_location = result.get("proxy_location")
            if proxy_location:
                location_usage[proxy_location] = (
                    location_usage.get(proxy_location, 0) + 1
                )
