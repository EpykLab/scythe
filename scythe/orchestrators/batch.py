...
class BatchOrchestrator(Orchestrator):
    ...
    def orchestrate_ttp(
        ...
        try:
            self.logger.info(
                f"Dividing {replications} executions into {num_batches} batches of {self.batch_config.batch_size}"
            )
            ...
            for batch_num in range(num_batches):
                ...
                batch_end = min(
                    (batch_num + 1) * self.batch_config.batch_size, replications
                )
                batch_size = batch_end - batch_start

                self.logger.info(
                    f"Executing batch {batch_num + 1}/{num_batches} (executions {batch_start + 1}-{batch_end})"
                )
                ...
                if batch_errors:
                    ...
                    if (
                        self.batch_config.retry_failed_batches
                        and batch_info["failed_executions"] > 0
                        and len(
                            [
                                b
                                for b in self.failed_batches
                                if b["batch_number"] == batch_num + 1
                            ]
                        )
                        <= self.batch_config.max_retries
                    ):
                        ...
                        all_results = [
                            ...
                            if not (
                                r.get("batch_number") == batch_num + 1
                ...
                if batch_num < num_batches - 1 and self.batch_config.batch_delay > 0:
                    self.logger.info(
                        f"Batch delay: {self.batch_config.batch_delay}s before next batch"
                    )
        ...
        all_results.sort(
            key=lambda r: (
                r.get("batch_number", 0),
                r.get("replication_number", 0),
                r.get("attempt_count", 1),
            )
        )
    ...
    def orchestrate_journey(
        ...
        try:
            ...
            for batch_num in range(num_batches):
                ...
                batch_info = {
                    ...
                    "batch_size": batch_size,
                    "successful_executions": sum(
                        1 for r in batch_results if r.get("overall_success", False)
                    ),
                    "failed_executions": sum(
                        1 for r in batch_results if not r.get("overall_success", False)
                    ),
                    "errors": batch_errors,
                }
                ...
                if batch_errors:
                    ...
                    if (
                        ...
                        all_results = [
                            ...
                            if not (
                                r.get("batch_number") == batch_num + 1
                                and not r.get("overall_success", False)
                            )
                        ]
                        all_results.extend(retry_results)
                        all_errors.extend(retry_errors)
        ...
        total_executions = len(all_results)
        successful_executions = sum(
            1 for r in all_results if r.get("overall_success", False)
        )
    ...
    def _execute_ttp_batch(
        ...
        **kwargs,
        ...
        max_workers = min(
            self.max_workers, batch_size, self.batch_config.max_concurrent_batches
        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for context in contexts:
                future = executor.submit(
                    self._execute_single_ttp_in_batch,
                    ttp,
                    target_url,
                    context,
                    **kwargs,
            ...
            for future in as_completed(future_to_context, timeout=self.timeout):
                ...
                try:
                    ...
                    self.logger.info(
                        f"  {status} Batch {batch_number + 1} execution {context.metadata['batch_position']}/{batch_size}"
                    )
    ...
    def _execute_journey_batch(
        ...
        **kwargs,
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for context in contexts:
                future = executor.submit(
                    self._execute_single_journey_in_batch,
                    journey,
                    target_url,
                    context,
                    **kwargs,
    ...
    def _execute_single_ttp_in_batch(
        self, ttp: TTP, target_url: str, context: ExecutionContext, **kwargs
    ) -> Dict[str, Any]:
    ...
    def _execute_single_journey_in_batch(
        self, journey: Journey, target_url: str, context: ExecutionContext, **kwargs
    ) -> Dict[str, Any]:
        ...
        with self.execution_lock:
            ...
            for context in self.active_executions.values():
                ...
                batch_groups[batch_num].append(
                    {
                        "execution_id": context.execution_id,
                        "batch_position": context.metadata.get(
