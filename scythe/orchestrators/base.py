...
class Orchestrator(ABC):
    ...
    def orchestrate_mixed(
        ...
        **kwargs,
        ...
        if len(target_urls) == 1:
            ...
            raise ValueError(
                "Number of target URLs must be 1 or equal to number of tests"
            )
        ...
        successful_executions = sum(
            ...
            if r.get("success", False) or r.get("overall_success", False)
        )
    ...
    def _log_orchestration_start(
        self, test_name: str, test_type: str, replications: int, target_url: str
    ):
    ...
    def _log_orchestration_end(self, result: OrchestrationResult):
        ...
        self.logger.info(
            f"Executions: {summary['executions']['successful']}/{summary['executions']['total']} successful ({summary['executions']['success_rate']})"
        )
...
class ExecutionContext:
    ...
    def is_successful(self) -> bool:
        ...
        if isinstance(self.result, dict):
            return self.result.get("success", False) or self.result.get(
