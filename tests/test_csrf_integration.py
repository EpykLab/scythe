...
class TestApiRequestActionCSRFIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.csrf = CSRFProtection(cookie_name="csrftoken", header_name="X-CSRF-Token")
        self.context = {
    ...
    @patch("scythe.journeys.actions.requests.Session")
    def test_api_request_keeps_explicit_referer(self, mock_session_class):
        """Test that explicit Referer is preserved when CSRF is injected."""
        mock_session = MagicMock()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.headers = {}
        ...
        action = ApiRequestAction(
            ...
            url="/api/items",
            body_json={"name": "test"},
            headers={"Referer": "https://custom.example.test/"},
            expected_status=200,
        )
        ...
        self.assertIn("X-CSRF-Token", headers)
        self.assertEqual(headers["Referer"], "https://custom.example.test/")
        self.assertTrue(result)
    ...
    def test_api_request_retries_on_403_csrf_failure(
    ...
    def test_api_request_no_csrf_injection_for_get(self, mock_session_class):
        ...
        action = ApiRequestAction(method="GET", url="/api/items", expected_status=200)
...
class TestJourneyCSRFIntegration(unittest.TestCase):
    ...
    def test_journey_initialization_with_csrf(self):
        ...
        journey = Journey(
            name="Test Journey", description="Test with CSRF", csrf_protection=csrf
        )
    ...
    def test_journey_executor_sets_csrf_in_context(self, mock_session_class):
        ...
        executor = JourneyExecutor(
            journey=journey, target_url="https://example.com", mode="API"
        )
...
class TestCSRFWithDifferentFrameworks(unittest.TestCase):
    ...
    def test_django_csrf_pattern(self, mock_session_class):
        ...
        csrf = CSRFProtection(
            extract_from="cookie", cookie_name="csrftoken", header_name="X-CSRFToken"
        )
        ...
        action = ApiRequestAction(
            method="POST", url="/api/items", body_json={"name": "test"}
        )
    ...
    def test_laravel_csrf_pattern(self, mock_session_class):
        ...
        csrf = CSRFProtection(
            extract_from="cookie", cookie_name="XSRF-TOKEN", header_name="X-XSRF-TOKEN"
        )
