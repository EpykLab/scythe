...
class TestRequestFloodingPayloads(unittest.TestCase):
    ...
    def test_seeded_rng_makes_payloads_deterministic(self):
        """Test seeded RNG produces stable user agent and delay sequences."""
        ttp_one = RequestFloodingTTP(
            target_endpoints=["/api/test"],
            request_count=5,
            payload_data={"x": 1},
            execution_mode="api",
        ...
        payloads_one = list(ttp_one.get_payloads())
        payloads_two = list(ttp_two.get_payloads())

        self.assertEqual(
            [p["user_agent"] for p in payloads_one],
            [p["user_agent"] for p in payloads_two],
        )
        self.assertEqual(
            [p["delay"] for p in payloads_one],
            [p["delay"] for p in payloads_two],
        )
