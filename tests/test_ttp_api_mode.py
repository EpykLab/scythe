...
class TestLoginBruteforceTTPAPIMode(unittest.TestCase):
    ...
    def test_login_bruteforce_execute_step_api(self):
        ...
        self.assertEqual(
            call_args[1]["json"], {"username": "admin", "password": "testpass"}
        )
    ...
    def test_login_bruteforce_verify_result_api_failure(self):
        ...
        ttp = LoginBruteforceTTP(
            ...
            execution_mode="api",
            api_endpoint="/api/login",
            success_indicators={"status_code": 200, "response_not_contains": "invalid"},
        )
...
class TestSQLInjectionTTPAPIMode(unittest.TestCase):
    ...
    def setUp(self):
        self.payload_gen = StaticPayloadGenerator(
    ...
    def test_input_field_injector_full_form_payload_is_not_shared(self):
        """Default/form payload should not leak across calls or instances."""
        ttp = InputFieldInjector(
            payload_generator=self.payload_gen,
            execution_mode="api",
            api_endpoint="/api/search",
        ...
        context = {"target_url": "http://test.com"}
        ttp.execute_step_api(mock_session, "first", context)
        ttp.execute_step_api(mock_session, "second", context)

        first_json = mock_session.request.call_args_list[0][1]["json"]
        second_json = mock_session.request.call_args_list[1][1]["json"]
        self.assertEqual(first_json["query"], "first")
        self.assertEqual(second_json["query"], "second")
        self.assertEqual(ttp.full_form_payload, {"fixed": "value"})
...
class TestTTPActionAPIMode(unittest.TestCase):
    ...
    def test_ttp_action_api_mode_unsupported_ttp(self):
        ...
        self.assertIn(
            "does not support API execution mode", action.get_result("error", "")
        )
    ...
    def test_ttp_action_reuses_existing_session(self):
        ...
        mock_driver = Mock()
        mock_session = Mock()
        context = {"target_url": "http://test.com", "requests_session": mock_session}
...
class TestBackwardCompatibility(unittest.TestCase):
    ...
    def test_sql_injection_backward_compatible(self):
        ...
        ttp2 = URLManipulation(
            payload_generator=payload_gen, target_url="http://test.com/items"
        )
