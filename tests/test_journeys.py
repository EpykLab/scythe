...
class TestStep(unittest.TestCase):
    ...
    def test_step_execution_results_reset_between_runs(self):
        """Step execution results should not leak across runs."""
        action = MockAction("Action 1", "First action", execution_result=True)
        step = Step("Test Step", "Test Description", actions=[action])

        with patch("logging.getLogger"):
            first_result = step.execute(self.mock_driver, self.context)
            second_result = step.execute(self.mock_driver, self.context)
...
class TestJourney(unittest.TestCase):
    ...
    def test_journey_requires_authentication(self):
        ...
        journey_with_auth = Journey(
            "Test Journey", "Test Description", authentication=auth
        )
    ...
    def test_journey_execution_with_authentication(self):
        ...
        journey = Journey(
            "Test Journey", "Test Description", steps=[step], authentication=auth
        )
...
class TestClickAction(unittest.TestCase):
    ...
    def test_click_action_invalid_selector_type(self):
        ...
        self.assertFalse(result)
        self.assertEqual(
            action.get_result("error"), "Unsupported selector type: invalid"
        )
...
class TestFillFormAction(unittest.TestCase):
    ...
    def test_fill_form_success(self, mock_wait):
        ...
        mock_wait.return_value.until.side_effect = [
            self.mock_username_field,
            self.mock_password_field,
        ]

        field_data = {"#username": "{username}", "#password": "testpass"}
    ...
    def test_fill_form_partial_failure(self, mock_wait):
        ...
        field_data = {"#username": "testuser", "#password": "testpass"}
...
class TestWaitAction(unittest.TestCase):
    ...
    def test_wait_element_action_success(self, mock_wait):
        ...
        action = WaitAction(
            wait_type="element", selector="#target", condition="presence", duration=10.0
        )
    ...
    def test_wait_unsupported_type(self):
        ...
        self.assertFalse(result)
        self.assertEqual(
            action.get_result("error"), "Unsupported wait type: unsupported"
        )
...
class TestTTPAction(unittest.TestCase):
    ...
    def test_ttp_action_success(self):
        ...
        ttp = MockTTP(
            "Test TTP", "Test Description", should_succeed=[True, False, True]
        )
    ...
    def test_ttp_action_expect_fail(self):
        ...
        ttp = MockTTP(
            "Test TTP", "Test Description", should_succeed=[False, False, False]
        )
...
class TestAssertAction(unittest.TestCase):
    ...
    def test_assert_unsupported_type(self):
        ...
        self.assertFalse(result)
        self.assertEqual(
            action.get_result("error"), "Unsupported assertion type: unsupported"
        )
...
class TestJourneyExecutor(unittest.TestCase):
    ...
    def test_executor_successful_journey(self, mock_webdriver):
        ...
        executor = JourneyExecutor(
            journey=journey, target_url="http://test.com", headless=True
        )
    ...
    def test_executor_was_successful(self):
        ...
        executor = JourneyExecutor(
            journey=Journey("Test", "Test"), target_url="http://test.com"
        )
        ...
        executor.execution_results = {"overall_success": True, "expected_result": True}
        self.assertTrue(executor.was_successful())

        # Mock failed results
        executor.execution_results = {"overall_success": False, "expected_result": True}
        self.assertFalse(executor.was_successful())
    ...
    def test_executor_get_action_results(self):
        ...
        executor.execution_results = {
            "step_results": [
                ...
                {
                    "step_name": "Step 2",
                    "actions": [{"action_name": "Action 3", "actual": True}],
                },
            ]
        }
